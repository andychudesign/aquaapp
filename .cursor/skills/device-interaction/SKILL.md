...
**This is a SUBAGENT skill.** Invoke it via the Task tool when device verification is needed.
...
Requires Xcode device-interaction MCP tools (`DeviceInteractionStartSession`, `DeviceInteractionInstallAndRun`, `DeviceEventSynthesize`, `DeviceInteractionEndSession`). If these tools are unavailable, tell the user device verification cannot run in this environment.
