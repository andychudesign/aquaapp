...
2. If not found, explain the value of tracking these decisions: "I'd like to create a decision document that records which security settings were enabled, disabled, or deferred, and why. This helps future audits build on past decisions instead of re-evaluating from scratch." Then ask via `AskQuestion`: "Where should I place it?" Options: "Project root (next to .xcodeproj)", "docs/ subdirectory", or let the user type a custom path.
