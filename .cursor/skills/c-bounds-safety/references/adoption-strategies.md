...
Adoption has many sub-steps across many files. Use `TodoWrite` at three moments so no sub-step is forgotten while keeping the active task list focused.
...
After creating every file-level task, wire the dependency chain `files → 4 → each 5.x umbrella → 6` by calling `TodoWrite` with the appropriate `addBlockedBy`:
...
**Moment C — first action when picking up any `Adopt -fbounds-safety in <file>` task.** Before modifying the file, `TodoWrite` sub-tasks for it mirroring sub-steps 3.1, 3.2, 3.3 (omit if the user did not provide a way to run the tests), 3.4, 3.5a, 3.5b. Only mark the file-level task `in_progress` after its sub-tasks exist.
...
3. `TodoWrite` a task `Add Safe Wrapper for <funcName>` with a structured description like:
...
4. `TodoWrite addBlockedBy` so the wrapper task can't surface until its gating predecessor is done — `[<step 4 task ID>]` in full adoption; `[<3a Confirm Safe Wrapper application task ID>]` in header-only.
5. `TodoWrite addBlocks [<Safe Wrapper umbrella task ID>]` so the umbrella checkpoint waits for this wrapper.
...
- Ensure a per-file `Adopt -fbounds-safety in <file>` task exists for the skipped file. If Moment B has already run, it does; otherwise (the skip was declared upfront during §0) `TodoWrite` it now so every skip has the same task representation regardless of when it was declared. `TodoWrite` that task to `completed` with a one-line note `skipped: <reason>`. If Moment C sub-tasks already exist for the file, mark each `completed` with the same note.
- `TodoWrite` the §4 task to `completed` with a one-line note `skipped: file(s) <X, Y, …> not adopted; per-file flags retained for adopted files`. If the §4 task was already marked complete-with-note by a previous skip, append the new file to the running list (re-edit the note via `TodoWrite`).
...
Header-only adoption is bounded — three numbered steps, with §3 being an opt-in Safe Wrapper batch. Use `TodoWrite` once at the start so the user can see the plan and no step is silently dropped. Before any file is modified, create exactly these tasks:
...
Wire the chain with `TodoWrite addBlockedBy` so order is enforced and the milestone only surfaces at the end:
