...
description: >-
  Guide for the C -fbounds-safety language extension. Covers the language model,
  pointer annotations, adopting bounds-safety in existing C code, compiler build
  settings and modes, and runtime debugging of bounds violations. Use when working
  with, reading, reviewing, comparing, debugging, or analyzing C code that has
  adopted -fbounds-safety or wants to adopt it. Key syntax includes bounds
  annotations (__counted_by, __sized_by, __ended_by, __single, __indexable,
  __bidi_indexable, __unsafe_indexable, __null_terminated), helper functions
  (__unsafe_forge_bidi_indexable, __unsafe_forge_single), macros
  (__ptrcheck_abi_assume_single), or includes of "ptrcheck.h".
---
